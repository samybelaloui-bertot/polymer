...
export class SamyWeather extends PolymerElement {
  ...
  async _getWeatherOfCity() {
    ...
    this.weather = await fetch(`https://api.openweathermap.org/data/2.5/weather?id=${cityId}&appid=${key}`).then(response => response.json()).then(json => {
  ...
  _selectCity(event) {
    console.log(event); // this.idCitySelected = event.model.__data.item['code'];
...
window.customElements.define(SamyWeather.is, SamyWeather); //# sourceMappingURL=samy-weather.js.map